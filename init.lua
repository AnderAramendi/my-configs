...
-- Telescope
  ...
  config = function()
    ...
    telescope.setup({
      defaults = {
        -- Hide Unity .meta files
        file_ignore_patterns = { "%.meta$" },
      },
      pickers = {
        -- Optional: also hide via ripgrep glob in live_grep
        live_grep = {
          additional_args = function()
            return { "--glob", "!*.meta" }
          end,
        },
        -- DO NOT set find_command here; let Telescope auto-pick (fd/rg/find)
      },
    })
  end
},
  ...
  },
  {
    ...
    config = function()
      ...
      -- ---- OmniSharp direct setup (no setup_handlers needed) ----
    local lspconfig = require("lspconfig")
    local is_windows = vim.loop.os_uname().sysname == "Windows_NT"
    local mason_bin = vim.fn.stdpath("data") .. "/mason/bin/omnisharp" .. (is_windows and ".cmd" or "")

    lspconfig.omnisharp.setup({
      cmd = { mason_bin }, -- use Mason’s installed binary
  ...
  -- Now add your custom mapping
  local function opts(desc)
    return { desc = desc, buffer = bufnr, noremap = true, silent = true, nowait = true }
  end

  -- Shift+C = change root to node
  vim.keymap.set("n", "C", api.tree.change_root_to_node, opts("CD to node"))
end,
      view = {
        side = "right"
      }
...
-- Make <Esc> exit terminal insert mode (like VS Code)
vim.keymap.set("t", "<Esc>", [[<C-\><C-n>]], { noremap = true })

-- Ctrl+alt+t to open terminal in bottom split (20% height)
